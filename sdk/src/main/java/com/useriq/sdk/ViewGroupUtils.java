...
public class ViewGroupUtils {
    private static final ThreadLocal<Matrix> TMP_MATRIX = new ThreadLocal<Matrix>() {
        protected Matrix initialValue() {
            return new Matrix();
        }
    };

    private static final ThreadLocal<RectF> TMP_RECT = new ThreadLocal<RectF>() {
    ...
    private static void offsetDescendantRect(ViewGroup parent, View descendant, Rect rect) {
        Matrix m = TMP_MATRIX.get();
        m.reset();

        offsetDescendantMatrix(parent, descendant, m);

        RectF rectF = TMP_RECT.get();
        rectF.set(rect);
        m.mapRect(rectF);
