...
public abstract class  InboxLayoutBase <T extends View> extends FrameLayout {
    ...
    private DragState dragState = DragState.CANNOT_CLOSE;
    ...
    private int moveContent(int offsetY){
        ...
        if((realOffsetY <- closeDistance || realOffsetY > closeDistance && dragListener !=null) && dragState != DragState.CAN_CLOSE) {
            dragListener.onInboxClosing();
            dragState = DragState.CAN_CLOSE;
        } else if(dragState != DragState.CANNOT_CLOSE && realOffsetY < closeDistance && realOffsetY >- closeDistance) {
            dragListener.onInboxOpen();
            dragState = DragState.CANNOT_CLOSE;
        }
        /*
        * Draw Shadow
        * */
        switch(mCurrentMode){
            case PULL_FROM_END:
                mScrollView.drawBottomShadow(mScrollView.getScrollY()+mScrollView.getHeight()-realOffsetY,
                        mScrollView.getScrollY()+mScrollView.getHeight(), 40);
                break;
            case PULL_FROM_START:
            default:
                mScrollView.drawTopShadow(mScrollView.getScrollY(), -realOffsetY, 40);
                break;
        }
    ...
    public void setBackgroundScrollView(InboxStickyScrollView scrollView){
        mScrollView = scrollView;
    }

    public void setDragListener(OnDragStateChangeListener listener){
    ...
    private InboxStickyScrollView mScrollView;
    ...
    private Interpolator mInterpolator = new DecelerateInterpolator(2);
    ...
    public void closeWithAnim(){
        ...
        if(dragListener !=null && dragState != DragState.CAN_CLOSE) {
            dragState = DragState.CAN_CLOSE;
            dragListener.onInboxClosing();
        }
    ...
    Property<InboxLayoutBase, Integer> aHeight = new Property<InboxLayoutBase, Integer>(Integer.class, "mHeight") {
        ...
        public void set(InboxLayoutBase object, Integer value) {
            ...
            if(value == heightRange && IsStartAnim){
                ...
                if(dragListener != null) dragListener.onInboxOpen();
