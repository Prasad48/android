 ...
 * used by {@link StarRatingView}, Most calculations (about x,y、size etc.) is done here.
 *
...
public class StarModel {
     ...
     * @see  StarModel
     */
    private StarRatingView.VertexF firstVertex;
     ...
     * @see #starVertexes
     */
    private StarRatingView.VertexF[] vertexes;

    // endregion

    private void initAllVertexesToStandard() {
        if (firstVertex == null) {
            firstVertex = new StarRatingView.VertexF(starVertexes[0], starVertexes[1]);
        } else {
        ...
        if (vertexes == null) {
            vertexes = new StarRatingView.VertexF[10];
            vertexes[0] = firstVertex;

            for (int i = 1; i < 10; i++) {
                vertexes[i] = new StarRatingView.VertexF();
        ...
        StarRatingView.VertexF current = firstVertex;
        for (int i = 0; i < 5; i++) {
        ...
        StarRatingView.VertexF prevOuter = firstVertex;
        for (int i = 0; i < 5; i++) {
            StarRatingView.VertexF innerV = prevOuter.next;
     ...
     */
    public StarRatingView.VertexF getVertex(int index) {
